<script lang="ts">
	...
	$: {
		api.getUserData(data.players[0]).then((user) => {
			if (user !== null && user !== APIStatus.NoResponse) { player1 = user };
		});
	}
	
	$: {
		api.getUserData(data.players[1]).then((user) => {
			if (user !== null && user !== APIStatus.NoResponse) { player2 = user };
		});
	}
	
	let role = (user === data.players[0]) ? LobbyWinner.Player1 : LobbyWinner.Player2;
...
<div class="match" class:win={data.winner === role} class:lose={data.winner !== role && data.winner !== LobbyWinner.Tie}>
	<div class="players">
		<div class="info">
			{player1?.username}<div class="id">#{padIdentifier(parseInt(player1?.identifier))}</div>
		</div>
		<div class="player">
			<div class="avatar" class:pointer={user !== data.players[0]} on:click={() => goToProfile(data.players[0])}>
				<UserAvatar uuid={data.players[0]}></UserAvatar>
			</div>
		</div>
		<div class="score">
		...
		<div class="player">
			<div class="avatar" class:pointer={user !== data.players[1]} on:click={() => goToProfile(data.players[1])}>
				<UserAvatar uuid={data.players[1]}></UserAvatar>
			</div>
		</div>
		<div class="info">
